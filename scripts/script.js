const translations = {
	...
	ar: {
		...
		about: {
			...
			skills: {
				network: "الشبكات وأنظمة التشغيل",
...
document.addEventListener("DOMContentLoaded", () => {
	...
	function renderProjects(lang) {
		...
		grid.querySelectorAll(".project-card").forEach((card) => {
			...
			const project = projectsData.find(
				(p) => p.id === image?.dataset.projectId,
			);
			...
			const thumbs =
				card.querySelectorAll(".slider-thumb");
	...
	function ensureImageViewer() {
		...
		viewer.addEventListener("click", (e) => {
			...
			const action = e.target
				.closest("[data-viewer-action]")
				?.dataset.viewerAction;
	...
	function openImageViewer(images, index, alt) {
		ensureImageViewer();
		...
		image.src = currentViewerImages[currentViewerIndex];
		image.alt = alt || "";
	...
	function updateLightboxTransform() {
		...
		image.style.transform =
			`translate(${lightboxX}px, ${lightboxY}px) ` +
			`scale(${lightboxScale})`;
	...
	/*
	 * Close image viewer with Escape
	 */
